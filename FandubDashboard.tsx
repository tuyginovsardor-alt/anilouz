...
import { 
    ...
    Instagram, Send, Save, ChevronRight, Share2
} from 'lucide-react';
...
export const FandubDashboard: React.FC = () => {
    ...
    const navItems = [
        { id: 'overview', label: 'Asosiy', icon: <LayoutGrid size={18}/> },
        { id: 'content', label: 'Loyihalar', icon: <Film size={18}/> },
        { id: 'analytics', label: 'Statistika', icon: <TrendingUp size={18}/> },
        ...
        <div className="min-h-screen bg-[#050505] text-white flex flex-col lg:flex-row pb-20 lg:pb-0">
            ...
            <aside className="hidden lg:flex w-72 bg-[#0a0a0a] border-r border-white/5 p-6 flex-col sticky top-0 h-screen">
                <div className="flex flex-col items-center mb-10">
                    <div className="w-24 h-24 rounded-3xl p-1 bg-gradient-to-tr from-purple-600 to-orange-500 mb-4 shadow-2xl relative">
            ...
            <main className="flex-1 p-4 md:p-8 lg:p-12 overflow-y-auto">
                ...
                {activeTab === 'overview' && (
                    <div className="space-y-8 lg:space-y-10 animate-fade-in max-w-7xl mx-auto">
                        <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
                            <div>
                                <h1 className="text-3xl md:text-5xl font-black uppercase tracking-tighter">Studio Dashboard</h1>
                            ...
                            <button onClick={() => { setEditingProject(null); setIsUploadModalOpen(true); }} className="w-full sm:w-auto px-8 py-4 bg-white text-black rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2"><Plus size={18}/> Yangi Anime Yuklash</button>
                        </header>

                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                            <div className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2rem] relative overflow-hidden">
                            ...
                            <div className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2rem]">
                                <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-3">Obunachilar</p>
                            ...
                            <div className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2rem]">
                                <p className="text-[8px] font-black text-zinc-500 uppercase tracking-widest mb-3">Loyihalar</p>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-6 md:p-8">
                                <h4 className="text-base font-black uppercase tracking-tight mb-6 flex items-center gap-3"><TrendingUp className="text-green-500" size={18}/> So'nggi Daromadlar</h4>
                                    ...
                                    {earnings.slice(0, 5).map(e => (
                                        <div key={e.id} className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-transparent hover:border-white/10 transition-all">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="text-xs font-bold text-white uppercase">{e.source}</p>
                                                    <p className="text-[8px] text-zinc-600 uppercase">{new Date(e.created_at).toLocaleDateString()}</p>
                                                </div>
                                            </div>
                                            <span className="text-green-400 font-black">+$ {e.amount.toFixed(3)}</span>
                                        </div>
                                    ))}
                                    {earnings.length === 0 && <p className="text-center py-10 text-zinc-800 uppercase font-black text-[9px]">Ma'lumot topilmadi</p>}
                                </div>
                            </div>
                            <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-6 md:p-8">
                                <h4 className="text-base font-black uppercase tracking-tight mb-6 flex items-center gap-3"><BarChart3 className="text-blue-500" size={18}/> Mashhur Loyihalar</h4>
                                <div className="space-y-3">
                                    {myUploads.slice(0, 5).sort((a,b)=> (b.view_count||0) - (a.view_count||0)).map(m => (
                                        <div key={m.id} className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl border border-white/5">
                                            <img src={m.poster_url} className="w-10 h-14 rounded-xl object-cover shadow-xl" alt="" />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-xs font-black text-white uppercase truncate">{m.title}</p>
                                                <p className="text-[9px] text-zinc-500 mt-1 flex items-center gap-1"><Eye size={10}/> {(m.view_count || 0).toLocaleString()}</p>
                                            </div>
                                            <div className="h-1 w-16 bg-zinc-800 rounded-full overflow-hidden">
                                                <div className="h-full bg-orange-500" style={{ width: `${Math.min(100, (m.view_count || 0) / 100)}%` }}></div>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                {activeTab === 'content' && (
                    <div className="animate-fade-in space-y-6 max-w-7xl mx-auto">
                        ...
                        <div className="lg:hidden grid grid-cols-1 gap-4">
                            {myUploads.map(up => (
                                <div key={up.id} className="bg-[#0a0a0a] border border-white/5 rounded-3xl p-5 space-y-5">
                                    <div className="flex gap-4">
                                        <img src={up.poster_url} className="w-20 h-28 rounded-2xl object-cover shadow-2xl" alt="" />
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-sm font-black text-white uppercase truncate">{up.title}</h4>
                                            <p className="text-[8px] font-black text-zinc-600 uppercase mt-1 tracking-widest">{up.genre.split(',')[0]} • {up.year}</p>
                                            <div className="mt-3">
                                                <StatusBadge status={up.status} comment={up.admin_comment} />
                                            </div>
                                            <div className="mt-3 flex items-center gap-2 text-blue-500 font-black text-xs">
                                                <Eye size={12}/> {(up.view_count || 0).toLocaleString()}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex gap-2 pt-4 border-t border-white/5">
                                        <button onClick={() => handleEditProject(up)} className="flex-1 py-3.5 bg-white/5 text-zinc-300 rounded-xl font-black text-[9px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all active:scale-95"><Edit3 size={14}/> Tahrirlash</button>
                                        <button onClick={() => handleDeleteProject(up.id)} className="flex-1 py-3.5 bg-red-600/10 text-red-500 rounded-xl font-black text-[9px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 transition-all active:scale-95"><Trash2 size={14}/> O'chirish</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="hidden lg:block bg-[#0a0a0a] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
                            <table className="w-full text-left">
                                <thead className="bg-[#111] text-[9px] font-black text-zinc-600 uppercase tracking-[0.2em]">
                                    <tr>
                                        <th className="p-8">Anime</th>
                                    ...
                                    {myUploads.map(up => (
                                        <tr key={up.id} className="group hover:bg-white/5 transition-all">
                                            <td className="p-8 flex items-center gap-6">
                                                <img src={up.poster_url} className="w-16 h-24 rounded-2xl object-cover shadow-2xl border border-white/10" alt="" />
                                                <div>
                                                    <p className="text-base font-black text-white uppercase tracking-tight">{up.title}</p>
                                                    <p className="text-[10px] font-black text-zinc-500 uppercase mt-1.5">{up.genre} • {up.access_type}</p>
                                                </div>
                                            </td>
                                            <td className="p-8">
                                                <span className="flex items-center gap-2 font-black text-sm text-white"><Eye size={16} className="text-blue-500"/> {(up.view_count || 0).toLocaleString()}</span>
                                            </td>
                                            <td className="p-8">
                                                <StatusBadge status={up.status} comment={up.admin_comment} />
                                            </td>
                                            <td className="p-8 text-right">
                                                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                                    <button onClick={() => handleEditProject(up)} className="p-4 bg-white/5 hover:bg-orange-600 rounded-2xl text-zinc-500 hover:text-white transition-all"><Edit3 size={20}/></button>
                                                    <button onClick={() => handleDeleteProject(up.id)} className="p-4 bg-white/5 hover:bg-red-600 rounded-2xl text-zinc-500 hover:text-white transition-all"><Trash2 size={20}/></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                ...
                {activeTab === 'analytics' && (
                    <div className="animate-fade-in space-y-8 max-w-7xl mx-auto">
                        <h2 className="text-3xl font-black uppercase tracking-tighter mb-8">Batafsil Tahlil</h2>
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            <div className="lg:col-span-2 bg-[#0a0a0a] border border-white/5 rounded-[3rem] p-8 min-h-[400px] flex flex-col">
                                <h4 className="text-sm font-black uppercase tracking-[0.2em] text-zinc-500 mb-10 flex items-center justify-between">Haftalik Ko'rishlar <Clock size={16}/></h4>
                                <div className="flex-1 flex items-end gap-2 sm:gap-6 px-2">
                                    {[30, 60, 40, 85, 55, 75, 45].map((h, i) => (
                                        <div key={i} className="flex-1 bg-zinc-900 rounded-t-2xl relative group hover:bg-orange-600 transition-all" style={{ height: `${h}%` }}>
                                            <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-white text-black text-[9px] font-black px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-all">{(h*254).toLocaleString()}</div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="space-y-6">
                                <div className="bg-gradient-to-br from-blue-600/20 to-transparent border border-blue-500/20 p-8 rounded-[2.5rem]">
                                    ...
                                    <p className="text-xs text-zinc-500 mt-4">Sizning kanalingiz o'tgan oyga qaraganda tezroq rivojlanmoqda.</p>
                                </div>
                ...
                {activeTab === 'wallet' && (
                    <div className="animate-fade-in grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 max-w-7xl mx-auto">
                        <div className="space-y-8">
                             <div className="bg-gradient-to-br from-purple-600 to-indigo-700 p-10 rounded-[3rem] shadow-3xl relative overflow-hidden">
                                 ...
                                 <div className="grid grid-cols-2 gap-4">
                                     <div className="bg-black/20 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 text-center">
                                     ...
                                     <div className="bg-black/20 backdrop-blur-md p-6 rounded-[2rem] border border-white/10 text-center">
                                         <p className="text-[8px] font-black uppercase text-white/50 tracking-widest mb-1">Kutilmoqda</p>
                             ...
                             <div className="bg-[#0a0a0a] border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-3xl">
                                 ...
                                 <form onSubmit={handleWithdraw} className="space-y-8">
                                     <div>
                                         <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4 mb-2 block">Summa (USD)</label>
                                         <div className="relative">
                                             <div className="absolute left-6 top-1/2 -translate-y-1/2 text-2xl font-black text-purple-500">$</div>
                                             <input type="number" min="10" step="0.01" value={wAmount} onChange={e=>setWAmount(e.target.value)} placeholder="0.00" className="w-full bg-black border border-white/10 rounded-3xl py-6 pl-14 pr-8 text-white font-black text-3xl outline-none focus:border-purple-600" required />
                                         </div>
                                     </div>
                                     <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                         <div className="space-y-2">
                                             <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4">Karta Raqami</label>
                                             <input value={wCard} onChange={e=>setWCard(e.target.value)} placeholder="8600 ...." className="w-full bg-zinc-900 border border-white/10 rounded-2xl p-5 text-white font-mono tracking-widest outline-none focus:border-purple-600" required />
                                         </div>
                                         <div className="space-y-2">
                                             <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4">F.I.SH</label>
                                             <input value={wHolder} onChange={e=>setWHolder(e.target.value)} placeholder="ISM FAMILIYA" className="w-full bg-zinc-900 border border-white/10 rounded-2xl p-5 text-white uppercase font-black text-xs tracking-widest outline-none focus:border-purple-600" required />
                                         </div>
                                     </div>
                        ...
                        <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-10 h-fit shadow-2xl">
                            <h4 className="text-xl font-black uppercase tracking-tight text-white mb-8 border-l-4 border-blue-600 pl-4">Yechib olinganlar</h4>
                                ...
                                {withdrawals.map(w => (
                                    <div key={w.id} className="p-6 bg-white/5 rounded-[2rem] border border-white/5 flex justify-between items-center group hover:bg-white/10 transition-all">
                                        <div className="flex items-center gap-4">
                                            <div className="w-12 h-12 bg-zinc-800 rounded-2xl flex items-center justify-center text-zinc-400 group-hover:scale-110 transition-transform"><Clock size={20}/></div>
                                            <div>
                                                <p className="text-lg font-black text-white">$ {w.amount.toFixed(2)}</p>
                                                <p className="text-[8px] text-zinc-600 font-mono">**** {w.card_number.slice(-4)} • {new Date(w.created_at).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                        <div className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest border ${w.status==='approved' ? 'bg-green-600/10 text-green-500 border-green-500/20' : w.status==='rejected' ? 'bg-red-600/10 text-red-500 border-red-500/20' : 'bg-yellow-600/10 text-yellow-400 border-yellow-500/20'}`}>
                                            {w.status}
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="animate-fade-in max-w-4xl mx-auto">
                        <div className="bg-[#0a0a0a] border border-white/10 rounded-[3.5rem] p-8 md:p-14 shadow-3xl relative overflow-hidden">
                            ...
                            <form onSubmit={handleUpdateSettings} className="space-y-8">
                                <div className="space-y-3">
                                    <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4">Studio / Kanal Nomi</label>
                                    <input value={channel?.name || ''} onChange={e=>setChannel(channel?{...channel, name:e.target.value}:null)} className="w-full bg-zinc-900 border border-white/10 rounded-[2rem] p-6 text-white font-black text-xl md:text-2xl uppercase focus:border-orange-600 outline-none transition-all" />
                                </div>
                                <div className="space-y-3">
                                    <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4">Ma'lumot (Bio)</label>
                                    <textarea value={channel?.bio || ''} onChange={e=>setChannel(channel?{...channel, bio:e.target.value}:null)} className="w-full bg-zinc-900 border border-white/10 rounded-[2rem] p-8 text-white text-base h-48 resize-none font-medium leading-relaxed outline-none focus:border-orange-600 transition-all" placeholder="Studio faoliyati haqida..." />
                                </div>
                                
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4 flex items-center gap-2"><Send size={12} className="text-blue-500"/> Telegram Kanal</label>
                                        <input value={channel?.social_links?.telegram || ''} onChange={e=>setChannel(channel?{...channel, social_links:{...channel.social_links, telegram:e.target.value}}:null)} className="w-full bg-zinc-900 border border-white/10 rounded-2xl p-5 text-white text-xs font-mono" placeholder="t.me/kanal_nomi" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest ml-4 flex items-center gap-2"><Instagram size={12} className="text-pink-500"/> Instagram Profile</label>
                                        <input value={channel?.social_links?.instagram || ''} onChange={e=>setChannel(channel?{...channel, social_links:{...channel.social_links, instagram:e.target.value}}:null)} className="w-full bg-zinc-900 border border-white/10 rounded-2xl p-5 text-white text-xs font-mono" placeholder="instagram.com/user" />
                                    </div>
                                </div>

                                <button type="submit" className="w-full py-6 bg-orange-600 text-white rounded-[2rem] font-black uppercase text-xs tracking-[0.4em] shadow-2xl hover:bg-orange-500 transition-all active:scale-95 flex items-center justify-center gap-4"><Save size={24}/> O'zgarishlarni Saqlash</button>
                            </form>
                        </div>
                    </div>
                )}

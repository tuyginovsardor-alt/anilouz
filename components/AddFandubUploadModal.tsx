...
import { uploadPoster, uploadVideo } from '../services/dbService';
...
export const AddFandubUploadModal: React.FC<AddFandubUploadModalProps> = ({ onClose, onSave, isUploading }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (posterType === 'file' && posterFile) {
            // Uploading will be handled in parent for clean progress, or here
        }
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4">
            ...
            <form onSubmit={handleSubmit} className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-5xl rounded-[3rem] p-8 md:p-12 overflow-y-auto max-h-[90vh] animate-slide-in-up custom-scrollbar">
                <div className="flex justify-between items-center mb-10">
                    <h2 className="text-4xl font-black uppercase tracking-tighter text-white">Loyiha Yuklash</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="space-y-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-4">Sarlavha</label>
                        ...
                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-4">Poster Manbasi</label>
                            ...
                            {posterType === 'file' ? (
                                <div className="relative border-2 border-dashed border-zinc-800 rounded-3xl p-10 flex flex-col items-center justify-center hover:border-purple-600/50 transition-all cursor-pointer">
                                    {posterFile ? <img src={URL.createObjectURL(posterFile)} className="absolute inset-0 w-full h-full object-cover rounded-3xl opacity-40" /> : <ImageIcon size={40} className="text-zinc-700" />}
                                    <span className="text-[10px] font-black uppercase text-zinc-500 mt-2">{posterFile ? posterFile.name : 'Poster yuklang'}</span>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-4">Tavsif</label>
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button type="button" onClick={handleAddEpisode} className="text-purple-500 text-[10px] font-black uppercase">+ Qo'shish</button>
                        </div>

                        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                            {episodes.map((ep, idx) => (
                                <div key={idx} className="bg-zinc-900/50 border border-white/5 p-5 rounded-3xl space-y-4">
                                    ...
                                    {ep.type === 'file' ? (
                                        ...
                                        <input value={ep.source as string || ''} onChange={e=>handleEpisodeChange(idx, 'source', e.target.value)} className="w-full bg-black/40 border border-zinc-800 rounded-xl p-3 text-xs text-zinc-400" placeholder="Direct Video URL (MP4, HLS)..." />
                                    )}
                                </div>
                            ))}
                        </div>
                ...
                <div className="mt-12 flex gap-4">
                    <button type="button" onClick={onClose} className="flex-1 py-5 bg-zinc-900 text-zinc-500 rounded-2xl font-black uppercase text-xs">Bekor qilish</button>
                    <button type="submit" disabled={isUploading} className="flex-1 py-5 bg-purple-600 text-white rounded-2xl font-black uppercase text-xs shadow-xl flex items-center justify-center gap-3">

...
export const getMovies = async (): Promise<Movie[]> => {
    ...
    return (data || []).map((m: any) => ({ ...m, posterUrl: m.poster_url, videoUrl: m.video_url }));
};
...
export const addMovieToDB = async (movie: any) => {
    ...
    return { ...data, posterUrl: data.poster_url, videoUrl: data.video_url };
};
...
export const updateReview = async (id: number, rating: number, comment: string) => {
    const { error } = await supabase
        .from('reviews')
        .update({ rating, comment, updated_at: new Date().toISOString() } as any)
        .eq('id', id);
    if (error) throw error;
};
...
export const getProfileWithWallet = async (userId: string) => {
    ...
    const wallet = await getATCWallet(userId);
...
export const getSavedMovies = async (userId: string): Promise<Movie[]> => {
    ...
    return (data || []).map((d: any) => ({ ...d.movies, posterUrl: d.movies.poster_url, videoUrl: d.movies.video_url }));
};
...
export const giveGlobalBonus = async (amount: number, description: string) => {
    const { data, error } = await supabase.rpc('give_global_bonus', { amount_val: amount, desc: description });
    if (error) throw error;
    return data;
};
...
export const getRecentActivity = async (): Promise<ActivityLog[]> => {
    ...
    return (data || []).map((l: any) => ({ id: l.id, type: l.type, title: l.title, description: l.description, time: new Date(l.created_at).toLocaleTimeString() }));
};
...
export const getContestSettings = async (): Promise<any> => {
    const config = await getAppConfig();
    return {
        exchange_rate: config['atc_exchange_rate'],
        ad_reward_atc: config['atc_ad_reward'],
        daily_free_spins: config['atc_daily_spins'],
        wheel_config: JSON.parse(config['atc_wheel_config'] || '[]'),
        quiz_questions_count: config['atc_quiz_count'],
        quiz_passing_score: config['atc_quiz_score'],
        quiz_reward_spins: config['atc_quiz_reward']
    };
};

export const updateContestSetting = async (key: string, value: string) => {
    await updateAppConfig(`atc_${key}`, value);
};
...
export const createContestTask = async (t: any) => {
...
export const getArkSettings = async (): Promise<any> => {
    const config = await getAppConfig();
    return {
        game_status: config['ark_game_status'] || 'active',
        current_price: Number(config['ark_current_price'] || 300),
        start_message: config['ark_start_message'],
        wheel_config: JSON.parse(config['ark_wheel_config'] || '[]'),
        autopilot_config: JSON.parse(config['ark_autopilot_config'] || '{}'),
        market_schedule: JSON.parse(config['ark_market_schedule'] || '{}')
    };
};

export const updateArkSettings = async (key: string, value: string) => {
    await updateAppConfig(`ark_${key}`, value);
};
...
export const requestArkWithdrawal = async (userId: string, amount: number, cardNum: string, cardHolder: string) => {
    const { error } = await supabase.from('ark_withdrawals').insert({ user_id: userId, amount_ark: amount, card_number: cardNum, card_holder: cardHolder, status: 'pending' });
    if (error) throw error;
};
...
export const giveArkGlobalBonus = async (amount: number, message: string) => {
    const { error } = await supabase.rpc('give_ark_global_bonus', { amount_val: amount, msg: message });
    if (error) throw error;
};
